...
def parseOptions():
    ...
    optParser.add_option('-a', '--agent', action='store', metavar="A",
                         ...
                         help='Agent type (options are \'random\', \'value\' and \'q\', \'policy\' ,default %default)')
...
if __name__ == '__main__':
    ...
    if opts.agent == 'value':
        ...
        a = policyIterationAgent.PolicyIterationAgent(
            mdp, opts.discount, opts.iters)
    else:
        if not opts.manual:
    ...
    try:
        ...
        if not opts.manual and opts.agent in ('policy'):
            if opts.valueSteps:
                for i in range(opts.iters):
